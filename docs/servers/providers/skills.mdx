...
<Note>
Clients that speak the standard [Skills extension](/servers/extensions#skills) can additionally discover these providers' skills through `skills/list` and `skills/get`, without changing anything about the provider itself. See [Server Extensions](/servers/extensions#skills) for how to register `SkillsExtension`.
</Note>
